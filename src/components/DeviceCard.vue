<template>
  <div v-if="props.device" class="flex flex-col w-full">
    <strong v-if="props.device.name" class="text-xl text-center">
      {{ deviceName }}
    </strong>
    ...
    <p v-if="!props.device.lastEvent">Updated at <Time :moment="props.device.updatedAt" /></p>

    <p
      v-if="
        props.device.compiler &&
        props.device.compiler.latestFirmware.hash !== props.device.firmware.hash
      "
